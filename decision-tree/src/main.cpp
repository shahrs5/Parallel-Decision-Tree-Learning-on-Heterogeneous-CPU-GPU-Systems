...
static bool testGiniImpurity() {
    ...
    bool all_pass = true;
    { std::vector<int> l = {1,1,1,1}; float g = DecisionTree::computeGini(l);
      all_pass &= check("pure node → 0.0", approxEq(g, 0.0f)); }
    { std::vector<int> l = {0,0,1,1}; float g = DecisionTree::computeGini(l);
...
static bool testCSVLoader() {
    ...
    int n = loadCSV(tmp, X, y); std::remove(tmp.c_str());
...
int main() {
    ...
    results.push_back(runDatasetBenchmark("Iris",              "data/iris.csv",              5, 1));
    results.push_back(runDatasetBenchmark("Wine",              "data/wine.csv",              5, 1));
    results.push_back(runDatasetBenchmark("Breast Cancer",     "data/breast_cancer.csv",     7, 2));
    results.push_back(runDatasetBenchmark("Banknote Auth",     "data/banknote.csv",          5, 1));
